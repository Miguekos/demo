<template>
  <div>
    ...
    <!-- <q-list separator>
      <q-item
        v-for="(item, index) in getClientesCSUser"
        :key="index"
        clickable
        v-ripple
      >
...
export default {
  ...
  computed: {
    ...mapGetters("client", ["getClientesCSUser"])
  ...
  methods: {
    ...mapActions("client", ["callClienteCSUser"]),
    crearDataExport() {
      const arraysJson = this.getClientesCSUser[0];
      let keys = [];
      let values = [];
    ...
    exportTable() {
      ...
      const content = [this.columnsexport.map(col => wrapCsvValue(col.label))]
        .concat(
          this.getClientesCSUser.map(row =>
            this.columnsexport
              .map(col =>
                wrapCsvValue(
  ...
  async created() {
    ...
    // });
    console.log(this.$q.localStorage.getAll().UserDetalle);
    await this.callClienteCSUser(this.$q.localStorage.getAll().UserDetalle);
    // await this.crearDataExport();
    // this.dataexport = this.getClientesCSUser();
