<template>
  <div>
    ...
    <!-- <q-list separator>
      <q-item
        v-for="(item, index) in getClientesSUser"
        :key="index"
        clickable
        v-ripple
      >
...
export default {
  ...
  computed: {
    ...mapGetters("client", ["getClientesSUser"])
  ...
  methods: {
    ...mapActions("client", ["callClienteSUser"]),
    crearDataExport() {
      const arraysJson = this.getClientesSUser[0];
      let keys = [];
      let values = [];
  ...
  async created() {
    ...
    await this.callClienteSUser(this.$q.localStorage.getAll().UserDetalle);
    // await this.crearDataExport();
    // this.dataexport = this.getClientesSUser();
