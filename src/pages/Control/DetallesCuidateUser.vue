...
export default {
  ...
  // mixins: [myMixin],
  computed: {
    ...mapGetters("segui", ["getSeguimientosJefe"])
  ...
  methods: {
    ...mapActions("segui", ["callSeguiCuidateJefe"]),
    crearDataExport() {
      const arraysJson = this.getSeguimientosJefe[0];
      let keys = [];
      let values = [];
    ...
    exportTable() {
      ...
      const content = [this.columnsexport.map(col => wrapCsvValue(col.label))]
        .concat(
          this.getSeguimientosJefe.map(row =>
            this.columnsexport
              .map(col =>
                wrapCsvValue(
  ...
  async created() {
    ...
    await this.callSeguiCuidateJefe(this.$q.localStorage.getAll().UserDetalle);
    // this.$q.loading.hide();
  }
};
</script>
